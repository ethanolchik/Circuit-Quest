...
public class TraceTable
{
    ...
    /// This method is used to simplify the process of comparing trace tables by focusing on functional equivalence
    /// rather than row-by-row alignment.
    ///
    /// For example:
    /// - If inputCount = 2 and row = [0, 1, 0]:
    ///   - Input bits: [0, 1] → Binary: "01" → Integer: 1
    ///   - Output: 0
    ///   - Map entry: { 1 -> 0 }
    /// 
    /// The map allows efficient validation of the logical relationship between inputs and outputs,
    /// regardless of the order of input columns.
    ///
    /// </summary>
    /// <returns>
    /// A dictionary where:
    /// - The key is an integer representing the bit pattern of inputs (e.g., 01 -> 1).
    /// - The value is the output corresponding to that input pattern.
    /// </returns>
